...
class RLPlane(Entity):
    def __init__(self, position, **kwargs):
        ...
        self.velocity_vec = Vec3(0,0,0)
        self.missile_count = 20
        self.gun_ammo = 1000

    def apply_actions(self, pitch, yaw, throttle_change, fire):
        # Map RL outputs (-1 to 1) to game physics
        self.rotation_x += pitch * 3.5
        self.rotation_y += yaw * 3.3
        
        # Update position based on current forward vector and speed
        yaw_rad = math.radians(self.rotation_y)
        pitch_rad = math.radians(self.rotation_x)
        
        # Calculate 3D forward vector for flight
        forward = Vec3(
        ...
        # Guns implementation - fires if the 'fire' action value is above a low threshold
        if fire > 0.2 and self.gun_ammo > 0:
            self.gun_ammo -= 1
            target = enemy_planes[0] if ('enemy_planes' in globals() and enemy_planes) else None
            if target and distance(self.position, target.position) < 800:
                to_target = (target.position - self.position).normalized()
                if forward.dot(to_target) > 0.98: # Tight boresight check
                    target.take_damage(0.5)

        # Missile implementation - fires if the 'fire' action value is above a high threshold
        if fire > 0.7 and (time.time() - self.last_shot_time > 2.0) and self.missile_count > 0:
            # Missile logic implementation
            target = enemy_planes[0] if ('enemy_planes' in globals() and enemy_planes) else None
            
            if 'missiles' in globals() and 'missile_velocity' in globals():
                missile_pos = self.position + forward * 5 + Vec3(0, 1, 0)
                new_missile = Missile(
